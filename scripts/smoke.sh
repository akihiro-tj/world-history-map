...
check() {
  local manifest basemap cities status etag
  manifest="$(curl -fsS "$base/asset-manifest.json")" || fail "manifest が取得できません" || return 1
  basemap="$(jq -r '."data/basemap.pmtiles"' <<<"$manifest")"
  cities="$(jq -r '."data/cities.json"' <<<"$manifest")"

  curl -fsS -o /dev/null "$base/" || fail "トップページが取得できません" || return 1

  status="$(curl -sS -o /dev/null -w '%{http_code}' -r 0-16383 "$base$basemap")"
  [[ "$status" == 206 ]] || fail "ベースマップの Range 応答が 206 ではありません: $status" || return 1

  etag="$(curl -sS -D - -o /dev/null -r 0-99 "$base$basemap" | tr -d '\r' | awk -F': ' 'tolower($1) == "etag" { print $2 }')"
  [[ -n "$etag" && "$etag" != W/* ]] || fail "ベースマップの ETag が無いか、弱い ETag です: $etag" || return 1

  status="$(curl -sS -o /dev/null -w '%{http_code}' -r 999999999-1000000000 "$base$basemap")"

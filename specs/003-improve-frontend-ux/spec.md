...
**Created**: 2026-02-21
**Status**: Implemented
**Input**: User description: "フロントエンドの情報設計を見直してブラッシュアップしたい — 5つの改善（現在年の常設表示、コントロールバー統合、選択領土マップハイライト、モバイルInfoPanelボトムシート化、ビジュアル階層見直し）"
...
- **初回ヒントUI（OnboardingHint）**: 実装後に不要と判断し削除。アプリのUIが十分に直感的であるため

## Clarifications

### Session 2026-02-21

- Q: ボトムシートのスナップポイント動作は？ → A: 3段階スナップ（collapsed: ヘッダーのみ / half: 40vh / expanded: 90vh）。速度ベースのフリック検出付き
...
### User Story 4 - モバイルでの情報パネルが使いやすい (Priority: P3)
...
1. **Given** モバイル画面幅（768px未満）, **When** 領土をクリック, **Then** 情報パネルが画面下部からスライドアップして40vh（half snap）で表示される
2. **Given** ボトムシートがhalf状態, **When** ヘッダーを上方向にスワイプ, **Then** expanded（90vh）に遷移しバックドロップが表示される
3. **Given** ボトムシートがhalf状態, **When** ヘッダーを下方向にスワイプ, **Then** collapsed（ヘッダーのみ）に遷移する
4. **Given** ボトムシートがcollapsed状態, **When** ヘッダーを下方向にスワイプ, **Then** パネルが閉じる
5. **Given** ボトムシートがexpanded状態, **When** バックドロップをタップ, **Then** パネルが閉じる
6. **Given** デスクトップ画面幅（768px以上）, **When** 領土をクリック, **Then** 従来通り左側のサイドパネルとして表示される

---

### User Story 5 - UI要素の視覚的階層が明確 (Priority: P3)
...
- ボトムシートの高さがコンテンツ量に対して不足する場合は？→ half/expanded状態でシート内スクロールが可能。expanded（90vh）で十分な高さを確保
...
**モバイルInfoPanelボトムシート**
- **FR-009**: モバイル画面幅（768px未満）で領土情報パネルを3段階スナップのボトムシート形式（collapsed: ヘッダーのみ / half: 40vh / expanded: 90vh）で表示しなければならない
- **FR-010**: ボトムシートはヘッダー部分のスワイプでスナップ間を遷移し、collapsed状態からの下方向スワイプで閉じられなければならない
- **FR-011**: expanded状態ではバックドロップ（黒50%）を表示し、タップで閉じられなければならない
- **FR-012**: half/expanded状態ではコンテンツスクロールが可能でなければならない
- **FR-013**: デスクトップ画面幅では従来通りサイドパネル形式を維持しなければならない

**ビジュアル階層**
- **FR-014**: 主要ナビゲーション（年代表示・年代セレクター）と補助ツール（コントロールバー）に視覚的な差異を設けなければならない。補助ツールはアイコン色を控えめ化（text-white/60等）し、ボタン背景は不透明を維持する
...
- **SC-002**: モバイル端末（375px幅）で情報パネルがhalf状態でも、マップの60%以上が視認可能な状態が維持される

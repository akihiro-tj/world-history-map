...
1. **Given** アプリケーションを開いた状態, **When** 初期画面が表示される, **Then** デフォルトの年代の世界地図と領土境界が表示される
...
4. **Given** 地図が表示されている状態, **When** 領土の境界線を見る, **Then** 各領土が視覚的に区別できる
...
1. **Given** 地図が表示されている状態, **When** 年代選択UIを操作する, **Then** 選択可能な年代の一覧が表示される
...
2. **Given** 領土の説明が表示されている状態, **When** 説明内容を確認する, **Then** 領土名、時代背景、主要な出来事が含まれている
...
- **領土説明 (Territory Description)**: 領土の歴史的背景、主要な出来事、関連する年代へのリンクを含むテキスト情報

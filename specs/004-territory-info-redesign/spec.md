...
- Q: プロフィールの「備考」フィールドの用途は？ → A: 削除。5フィールド（首都・政体・指導者・人口・宗教）に限定する
...
- **マスターデータ**: Google スプレッドシートを正（Single Source of Truth）とする
- **データパイプライン**: スプレッドシートからデータを fetch し、JSON に変換して `public/` ディレクトリに保存するコマンドを作成する
- **人的レビュー**: データ再作成（ユーザーストーリー6）は、実装前にスプレッドシート上のデータの人的レビュー・承認を経てから進める
...
3. **前提** ユーザーが複数の領土を順番に閲覧する、**操作** 比較する、**結果** プロフィールフィールドがすべての領土で一貫した順序（首都 → 政体 → 指導者 → 人口 → 宗教）で表示される。
...
- **FR-006**: プロフィールフィールドは、すべての領土で一貫した順序（首都 → 政体 → 指導者 → 人口 → 宗教）で表示されなければならない（MUST）。
...
- **FR-016**: Google スプレッドシートをマスターデータとし、スプレッドシートから JSON へ変換して `public/` ディレクトリに保存するデータ同期コマンドを提供しなければならない（MUST）。
...
- **Profile**: 現在の非構造化 `facts` 配列を置き換える、オプションの名前付きフィールド（首都、政体、指導者、人口、宗教）を持つ構造化サブエンティティ。
...
- プロフィールフィールドのセット（首都、政体、指導者、人口、宗教）は初回リリースとして十分である。追加フィールドは将来のイテレーションで追加可能。

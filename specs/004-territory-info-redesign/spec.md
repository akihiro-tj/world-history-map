...
既存の領土情報データ（`public/data/descriptions/*.json`）をすべて削除し、**高校世界史の教科書に登場するレベルの主要な国家・帝国**に絞って新しい構造化フォーマットで再作成する。
...
3. **前提** ユーザーが複数の領土を順番に閲覧する、**操作** 比較する、**結果** プロフィールフィールドがすべての領土で一貫した順序（首都 → 政体 → 指導者 → 人口 → 宗教 → 備考）で表示される。
...
- **FR-003**: データモデルは、選択年におけるその領土の時間的状況を客観的事実のみで記述する文章形式の `context` フィールド（文字列）を含まなければならない（MUST）。
- **FR-004**: タイムラインの各キーイベントは、選択年に対して `past`（過去）、`current`（現在）、`future`（未来）として分類可能でなければならない（MUST）。
- **FR-005**: パネルは、データが利用できないプロフィールフィールド、文脈セクション、タイムラインセクションについて、プレースホルダーや "不明" 値を表示するのではなく、そのセクション自体を省略しなければならない（MUST）。
- **FR-006**: プロフィールフィールドは、すべての領土で一貫した順序（首都 → 政体 → 指導者 → 人口 → 宗教 → 備考）で表示されなければならない（MUST）。
...
- **Profile**: 現在の非構造化 `facts` 配列を置き換える、オプションの名前付きフィールド（首都、政体、指導者、人口、宗教、備考）を持つ構造化サブエンティティ。
...
- プロフィールフィールドのセット（首都、政体、指導者、人口、宗教、備考）は初回リリースとして十分である。追加フィールドは将来のイテレーションで追加可能。

...
- [ ] T027 [US6] 教科書レベルの主要領土データの CSV を生成し、ユーザーが Notion にインポートする — spec.md の選定基準に従い 50〜70 領土を選定（高校世界史教科書レベル、地域バランス、時代カバレッジ）。地域バッチ単位でエージェントに CSV 生成を委譲し、メインコンテキストで統合 CSV（`.cache/notion-territory-data.csv`）を作成する。"不明" は使用しない。`notion-territory-data` スキルを使用し、すべて日本語で記述（FR-015）。統合 CSV をユーザーに渡し、Notion UI から「Merge with CSV」でインポートしてもらう

...
- [x] T027 [US6] 教科書レベルの主要領土データの CSV を生成し、ユーザーが Notion にインポートする — spec.md の選定基準に従い 50〜70 領土を選定（高校世界史教科書レベル、地域バランス、時代カバレッジ）。地域バッチ単位でエージェントに CSV 生成を委譲し、メインコンテキストで統合 CSV（`.cache/territory-data.csv`）を作成する。"不明" は使用しない。`territory-data` スキルを使用し、すべて日本語で記述（FR-015）。統合 CSV をユーザーに渡し、Notion UI から「Merge with CSV」でインポートしてもらう
- [x] T028 [US6] 人的レビュー完了後に `pnpm pipeline sync-descriptions` を実行し、全 JSON ファイルを生成する — Notion 上のデータのレビュー・承認を経てからパイプラインを実行。`pnpm pipeline validate-descriptions` でバリデーションが全件通過することを確認。T008 で作成したサンプルデータは本番データで上書きされる
...
- [X] T029 全体バリデーションを実行する: `pnpm test && pnpm check` — 全ユニットテスト、コンポーネントテスト、リンティングが通過すること
...
# Phase 9 完了後（本番データ作成）:
Task T027: "CSV を生成しユーザーが Notion にインポートする"
...
- **T027（本番データ）**: 地域バッチ単位でエージェントが CSV を生成し、メインコンテキストで統合。ユーザーが Notion UI から CSV インポート

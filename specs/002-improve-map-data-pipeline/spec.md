...
現在、`upload-to-r2.sh` はほとんどのファイルが変更されていなくても、毎回すべてのファイルをストレージバケットにアップロードする。53ファイル、527MBのPMTilesデータに対して、これは無駄である。
...
1. **前提** 以前のデプロイが存在する状態で、**操作** 53ファイル中3ファイルが変更された場合、**結果** その3ファイルのみがアップロードされる。
...
- 変換中にディスク容量が不足した場合 → tippecanoe/tile-join がエラー終了し、パイプラインの既存エラーハンドリングで捕捉される。チェックポイント機能により、ディスク空き容量確保後に失敗したステージから再開可能。専用のディスク容量事前チェックは実装しない（YAGNI）。

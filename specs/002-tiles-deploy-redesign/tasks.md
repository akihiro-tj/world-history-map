...
- [x] T062 移行猶予期間（最低 2 週間）が経過した後、`apps/worker/src/index.ts` の `/manifest.json` ハンドラを削除し、Worker を再 deploy
...
- [x] T068 quickstart.md の「検証ポイント」全項目を実機で実行し、すべてパスすることを確認（frontend-only main マージで本番 deploy が走ること、descriptions が改変されていないことを含む）

...
**実装**: `src/pipeline/stages/fetch.ts`
...
**実装**: `src/pipeline/stages/merge.ts`
...
**実装**: `src/pipeline/stages/validate.ts`, `src/pipeline/validation/geojson.ts`
...
**実装**: `src/pipeline/stages/convert.ts`
...
**実装**: `src/pipeline/stages/prepare.ts`
...
**実装**: `src/pipeline/stages/index-gen.ts`

### ステージ 7: upload

ローカルの SHA-256 ハッシュとデプロイメントマニフェストの `metadata` を比較し、変更されたファイルのみを `wrangler r2 object put` で Cloudflare R2 にアップロードする。

- **前提**: `CLOUDFLARE_API_TOKEN` 環境変数または `wrangler login` で認証が必要
- **バケット**: `world-history-map`
- **スキップ**: `--skip-upload` フラグで省略可能

**実装**: `src/pipeline/stages/upload.ts`

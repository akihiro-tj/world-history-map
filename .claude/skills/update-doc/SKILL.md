...
各ドキュメントを開き、先頭の `> Last updated: YYYY-MM-DDTHH:MM:SS+09:00`（ISO 8601 タイムゾーンオフセット付き）を読み取る。

- ファイルが存在しタイムスタンプがある → **差分更新対象**
- ファイルが存在しない、またはタイムスタンプが見つからない → **新規生成対象**

タイムスタンプは git log の `--since` にそのまま渡せる精度で記録する。日付だけだと同日内の更新前後を区別できないため必ず時刻まで含めること。
...
例: `git log --since='2026-04-17T14:30:00+09:00' --pretty=format:"%h %s" -- apps/pipeline/`
...
更新後、各ファイルの `Last updated` を書き込み時点の ISO 8601 タイムスタンプ（タイムゾーンオフセット付き、例: `2026-04-17T22:00:00+09:00`）に揃える。

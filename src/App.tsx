  ...
  return (
    <main className="relative h-screen w-screen overflow-hidden">
      ...
      {!isLoading && years.length > 0 && (
        <div className="absolute bottom-4 left-1/2 z-20 w-full max-w-2xl -translate-x-1/2 rounded-lg bg-white/95 shadow-lg backdrop-blur-sm">
      ...
      <footer className="absolute bottom-4 right-4 z-20">
        <div className="flex items-center gap-3 rounded-lg bg-white px-3 py-2.5 shadow-md">
          <button
            ...
            className="group relative h-6 w-6 text-gray-600 hover:text-gray-900"
          >
            {/* Outline version (default) */}
            <svg className="absolute inset-0 transition-opacity duration-200 group-hover:opacity-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" aria-hidden="true">
              <path strokeLinecap="round" strokeLinejoin="round" d="M11.25 11.25l.041-.02a.75.75 0 011.063.852l-.708 2.836a.75.75 0 001.063.853l.041-.021M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-9-3.75h.008v.008H12V8.25z" />
            </svg>
            {/* Filled version (hover) */}
            <svg className="absolute inset-0 opacity-0 transition-opacity duration-200 group-hover:opacity-100" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">

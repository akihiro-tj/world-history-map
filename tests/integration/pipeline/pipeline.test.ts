...
vi.mock('@/pipeline/config.ts', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@/pipeline/config.ts')>();
  return {
    ...actual,
...
import { mergeByName } from '@/pipeline/stages/merge.ts';
import { prepareTile } from '@/pipeline/stages/prepare.ts';
import { createInitialState, loadState, saveState } from '@/pipeline/state/checkpoint.ts';
